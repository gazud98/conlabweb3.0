...
require_once 'DriversFactory.php';
require_once './core/sessions/BaseSessionDriver.php';

class Session extends DriversFactory{

    /*** Available drivers */
    const FILES_DRIVER = 'Files';
    const DATABASE_DRIVER = 'Database';

    /** * @var array Internal session variables*/
    private static $reservedSessionKeys = array('_temp_bag','_flashes_bag','_last_regenerate');
    private $driversFactory;

    /** * Session constructor.** @param array $config Configuration array*/
    public function __construct(array $config){

        $this->driversFactory = new DriversFactory();

        if (strpos(PHP_SAPI, 'cli') === 0 || defined('STDIN')) {
            trigger_error('Session cannot start in CLI mode.', E_USER_NOTICE);
            return;
        }

        if ((bool)ini_get('session.auto_start')) {
            trigger_error('"session.auto_start" INI setting is enabled; Session cannot start.', E_USER_ERROR);
            return;
        }
        ...
        if (!$this->isAjaxRequest() && ($regenerate_time = $config['regenerate_time']) > 0) {
            ...
            if (PHP_VERSION_ID < 70300) {
                ...
                setcookie($config['cookie_name'], session_id(), [
                    'expires' => $config['cookie_lifetime'] === 0 ? 0 : $config['cookie_lifetime'] + time(),
    ...
    /*** __get()* * @param string $key Session data key * @return mixed */
    public function __get($key){
        if (array_key_exists($key, $_SESSION)) { return $_SESSION[$key];
        }elseif ($key === 'session_id'){ return session_id(); }
        return null;
    }

    /*** __isset() * * @param string $key Session data key* @return bool*/
    public function __isset($key){
        if ($key === 'session_id'){ return (session_status() === PHP_SESSION_ACTIVE); }
        return isset($_SESSION[$key]);
    }

    /** * __set()** @param string $key Session data key * @param mixed $value Session data value*/
    ...
    public function userdata($key = NULL){
		...
		}elseif (empty($_SESSION)){ return array(); }
		...
		foreach (array_keys($_SESSION) as $key){
			if ( ! in_array($key, $_exclude, TRUE)){ $userdata[$key] = $_SESSION[$key]; }
		}

		return $userdata;
	}
     
    //Matriz que contiene las claves de todos los elementos "flashdata".
    public function get_flash_keys(){
        
		if (!isset($_SESSION['_flashes_bag'])){ return array(); }
		$keys = array();
		foreach (array_keys($_SESSION['_flashes_bag']) as $key){
			is_int($_SESSION['_flashes_bag'][$key]) OR $keys[] = $key; }
		return $keys;
	}
    
    //	Matriz que contiene las claves de todos los elementos "tempdata".
    public function get_temp_keys(){

		if ( ! isset($_SESSION['_temp_bag'])){ return array(); }
		$keys = array();
		foreach (array_keys($_SESSION['_temp_bag']) as $key){
			is_int($_SESSION['_temp_bag'][$key]) && $keys[] = $key; }
		return $keys;
	}
    ...
    public function flashdata($index = null){
        ...
        if (!isset($_SESSION['_flashes_bag'])){ return null;  }
        if ($index !== null) {
    ...
    public function tempdata($index = null){
        ...
        if (!isset($_SESSION['_temp_bag'])){ return null; }
        if ($index !== null) {
    ...
    public function setUserdata($index, $value = null){
        if (is_array($index)){ foreach ($index as $key => $value){ $_SESSION[$key] = $value; }
