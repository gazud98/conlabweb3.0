...
class Security{
	...
	//solucionar errores de esta libreria
	...
	public function csrf_verify(){
		...
		if ($exclude_uris = csrf_exclude_uris){
			foreach ($exclude_uris as $excluded){
				if (preg_match('#^'.$excluded.'$#i'.(UTF8_ENABLED ? 'u' : ''), $this->uri_string()))
				{
					return $this;
				}
			}
	...
	public function csrf_set_cookie()
	{
		...
		$secure_cookie = (bool) cookie_secure;
    ...
    private function remove_invisible_characters($str, $url_encoded = TRUE){
		...
		do{ $str = preg_replace($non_displayables, '', $str, -1, $count); }
		while ($count);
		return $str;
	}
