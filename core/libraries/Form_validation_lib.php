...
require_once 'config/lang/'.lang_validation.'/form_validation_lang.php';
require_once 'core/platform/Input.php';

class Form_validation_lib extends Input{
	...
	public function __construct($rules = array()){
		...
		if ($this->input === null){ return $this->input = new Input(); } 
	...
	public function set_class_parent($index){ $this->cw = $index; }
	...
	public function set_rules($field, $label = '', $rules = array(), $errors = array()){
		...
		if ($this->input->method() !== 'post' && empty($this->validation_data)){ return $this; }
		...
		if (is_array($field)){
			foreach ($field as $row){
				...
				if ( ! isset($row['field'], $row['rules'])){ continue; }
		...
		if (!is_string($field) OR $field === '' OR empty($rules)){ return $this; 
		}elseif (!is_array($rules)){
			// BC: Convert pipe-separated rules string to an array
			if (!is_string($rules)){ return $this; }
	...
	public function set_data(array $data){	
		...
		if (!empty($data)){ $this->validation_data = $data;}
		return $this;
	...
	public function error($field, $prefix = '', $suffix = ''){
		if (empty($this->_field_data[$field]['error'])){ return ''; }
	...
	public function run($group = ''){
		...
		if (count($this->_field_data) === 0){
			...
			if (empty($group)){
				...
				$group = trim($this->cw->uri->ruri_string(), '/');
				isset($this->_config_rules[$group]) OR $group = $this->cw->router->class.'/'.$this->cw->router->method;
			}
	...
	protected function _execute($row, $rules, $postdata = NULL, $cycles = 0){
		...
		if (is_array($postdata) && !empty($postdata)){ foreach ($postdata as $key => $val){ $this->_execute($row, $rules, $val, $key); }	return;	}
		...
		foreach ($rules as $rule){
			...
			if ($callback OR $callable !== FALSE){
				...
				if ($callback){
					if (!method_exists($this->cw, $rule)){
						//log_message('debug', 'Unable to find callback validation rule: '.$rule);
						$result = FALSE; 
					}else{
						// Run the function and grab the result
						if (self::$class === null) { self::$class = new $this->cw();}
						$call = self::$class->$rule();
						if(is_array($call)){
							if($call['result']==FALSE){
								$this->set_message($row['field'],$call['message']);
								$result = FALSE; 
								//$this->_field_data[$row['field']]['postdata'] = $result;
								$this->_field_data[$row['field']]['errors'][$rule] = $call['message'];
							}else{
								$result = TRUE; 
							}
						}
			...
			if ($result === FALSE){  
				...
				if (!is_string($rule)){
					$line = LANG['form_validation_error_message_not_set'].'(Anonymous function)';
				}else{  
	...
	protected function _get_error_message($rule, $field){
		...
		elseif (FALSE !== ($line = LANG[$rule]))
		{  
			return $line;
		}

		return LANG['form_validation_error_message_not_set'].'('.$rule.')'; 
	}
	...
	protected function _translate_fieldname($fieldname)
	{
		...
		if (sscanf($fieldname, 'lang:%s', $line) === 1 && FALSE === ($fieldname = LANG[$line]))
		{
			return $line;
		}
	...
	public function is_unique($str, $field)
	{
		...
		$this->db         = $this->conectar->conexioPDO();
		return isset($this->db)? ($this->db->get_row($table, array($field => $str)) == 0): FALSE;
	}
	...
	public function valid_url($str)
	{
		...
		if (preg_match('/^\[([^\]]+)\]/', $str, $matches) && ! parent::is_php('7') && filter_var($matches[1], FILTER_VALIDATE_IP, FILTER_FLAG_IPV6) !== FALSE)
		{
	...
	{
		return $this->cw->security->strip_image_tags($str);
	}
