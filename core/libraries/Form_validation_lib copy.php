...
class Form_validation_libf extends Input{
	...
	public function set_class_parent($index){ $this->indexing_son = $index; }

	public function set_rules($field, $label = '', $rules = array(), $errors = array()){
		// No reason to set rules if we have no POST data
		// or a validation array has not been specified
		if (parent::method() !== 'post' && empty($this->validation_data)) {
			return $this;
		}
		...
		foreach ($rules as $rule) {
			...
			if ($callback or $callable !== FALSE) {
				if ($callback) {
					...
					if (!method_exists($this->indexing_son, $rule)) {
						//log_message('debug', 'Unable to find callback validation rule: '.$rule);
						$result = FALSE;
					} else {
						// Run the function and grab the result
						if (self::$class === null) { self::$class = new $this->indexing_son();}
	...
	protected function _get_error_message($rule, $field)
	{
		...
		} elseif (FALSE !== ($line = LANG['required'])) {
			return $line;
		}
	...
	{
		return parent::strip_image_tags($str); //security
	}
