...
class ConsumosModel extends ModeloBase
{
    ...
    public function get_name_sede_by_id($where)
    {
        ...
        return  $this->conexioPDO->get_row(prefixdb . "sedes", $where)['nombre'];
    }
    ...
    public function get_info_kits()
    {
        ...
        $query       =   $this->conexioPDO->get(dblab . 'info_kit');
        return $query;
    }

    function get_costo_referencia_by_id($where)
    {

        $sql = "SELECT ex.costo ,ex.referencia FROM " . dblab . "examenes ex, " . dblab . "info_kit ik WHERE ik.id_examenes = ex.id_examenes   and ik.id_info_kit =" . $where;
    ...
    function get_list_live_kit($where, $pint)
    {
        $filtro = " and 1=1"; // Filtro base
        if ($where == 'all') {
            $filtro .= " AND ilk.id_sedes LIKE '%'";
        } else {
            if ($where != "") {
                $filtro .= " AND ilk.id_sedes  = '" . $where . "'";
            }
        }

        if ($pint != '') {
            $filtro .= " AND ik.name_examen LIKE '%" . $pint . "%'";
        }


        $sql = "SELECT ik.name_examen, ilk.id_info_live_kit, ilk.estado, 
                            ik.codigo_cups, ilk.date_start, ilk.end_live, ilk.fabricante, 
                        ...
                        WHERE ilk.estado = 1 " . $filtro . " ORDER BY id_info_live_kit DESC";
    ...
    function get_live_kit_by_id($id)
    {
        ...
        $sql = "SELECT ik.name_examen, ilk.id_info_live_kit, ilk.estado, 
                           ...
                           ilk.bonificables, ilk.fabricante,  se.nombre as sede
                    FROM " . dblab . "info_live_kit as ilk
                    INNER JOIN " . dblab . "info_kit as ik ON (ilk.id_info_kit = ik.id_info_kit) 
    ...
    public function update_fecha_expiration($fechaActual, $id)
    {

        $sql = "UPDATE " . dblab . "info_live_kit
         SET end_live ='" . $fechaActual . "' WHERE id_info_live_kit = " . $id;
    ...
    public function get_table_costos_indirectos($query)
    {
        ...
        // Fetch records
        $sql = "SELECT id_costos_indirectos_fabricacion, se.nombre as 'sede', dc.descripcion, cif.valor, cif.ano_mes, cif.motivo_costos                       
                    FROM " . dblab . $table . " as cif
                    INNER JOIN sedes as se  
    ...
    public function get_table_mano_obra($query)
    {
        ...
        $sql = "SELECT md.id_mano_obra_directa, al.descripcion as 'arealab',md.fecha,  
                           md.valor, md.tiempo_dedicacion, se.nombre as 'sede', sec.nombre  as 'seccion', 
