...
if ($conetar->connect_errno) {
    ...
    $id = trim($_POST['idreg']);
    $regfiscal = trim($_POST['regfiscal']);


    $cadena2="select idreg from u116753122_cw3completa.regimen_fiscal_config WHERE idreg = '$id'";
    $resultadP2=$conetar->query($cadena2);
    $numerfiles2 = mysqli_num_rows($resultadP2);
    if($numerfiles2<1){
        $cadena = "insert into u116753122_cw3completa.regimen_fiscal_config(descripcion) 
        values('" . $regfiscal . "')";
        $resultado = mysqli_query($conetar, $cadena);
    }else{
        $cadena="update u116753122_cw3completa.regimen_fiscal_config set
                                    descripcion='".$regfiscal."'
                                    where idreg = '".$id."'";
