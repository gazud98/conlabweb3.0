...
<table class="table table-striped table-hover table-head-fixed text-nowrap table-sm" id="table-condiciones_pagos" style="
                    font-size: 15px;
                    text-align: center;
...
<script>
    function exportarExcel() {
        ...
        var tabla = document.getElementById('table-condiciones_pagos');
    ...
    $(document).ready(function() {
        miDataTable = $('#table-condiciones_pagos').DataTable({
            language: {
            ...
            "responsive": true,
            "ajax": {
                url: 'https://cw3.tierramontemariana.org/apps/condiciones_pagos/show_data.php', // Página PHP que devuelve los datos en formato JSON
            ...
            "columns": [{
                ...
                "render": function(data, type, full, meta) {
                    if (full.nombre === "1") {
                        return "Pago De Contado" + " - " + full.numero + " días";
                    } else {
                        return "Credito" + " - " + full.numero + " días";
                    }
                }
            }, {
                "data": "estado",
                    ...
                    success: function(respuesta) {
                        if (respuesta == 1) {
                            Swal.fire({
                                icon: 'error',
                                title: 'Oops...',
                                text: '¡Esta Condicion de Pago ya existe!',
                                footer: 'Crea una Condicion de Pago con un nuevo nombre'
                            })
                        } else {
                            Swal.fire({
                                position: 'top',
    ...
    function editar(id) {
        ...
        $.ajax({
            type: 'GET',
            url: 'https://cw3.tierramontemariana.org/apps/condiciones_pagos/process_data.php',
            data: {
                id: id
            },
            dataType: 'json',
            success: function(response) {
                ...
                var nombrec = response.nombre;
                
                $('#numeroedit').val(response.numero);
