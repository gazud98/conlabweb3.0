...
?>
    ...
    <script>
        $(document).ready(function() {
            dataTableAlert = $('.table-all-mant').DataTable({
                dom: 'Bfrtip',
                buttons: [
                    'print'
                ],
                ...
                // ... Otras opciones ...
                "ajax": {
                    url: 'https://cw3.tierramontemariana.org/apps/consultamantenimiento/mostrar-2.php', // Página PHP que devuelve los datos en formato JSON
                ...
                "columns": [{
                    ...
                    },
                    {
                        ...
                        "render": function(data, type, full, meta) {
                            ...
                            /*const fechaReferencia = new Date(full.fecha_correctivo);
                            const fechaActual = new Date('d/m/Y');
                            const diferenciaMilisegundos = fechaReferencia - fechaActual;
                            const diasDiferencia = Math.ceil(diferenciaMilisegundos / (1000 * 60 * 60 * 24));

                            if (diasDiferencia > 0) {
                              console.log(fechaActual)
                              return `<span class="badge badge-success">Faltan ${diasDiferencia} días</span>`;
                            } else if (diasDiferencia < 0) {
                              return `<span class="badge badge-danger">Atrasado por ${-diasDiferencia} días.</span>`;
                            } else {
                              console.log(fechaActual)
                              return `<span class="badge badge-warning">Hoy es la fecha.</span>`;
                            }*/
            ...
            $('#button-fil').click(function() {
                dataTableAlert.ajax.reload(); // Recarga los datos de la tabla con los nuevos parámetros
            });
        })
        ...
        function loadModalRepAll(id, aux) {
            $('#contentModalRep').load('https://cw3.tierramontemariana.org/apps/consultamantenimiento/modal-rep.php', {
                id: id,
                aux: 'A'
            });
        }
