...
if ($conetar->connect_errno) {
    ...
    if ($modeeditstatus == "D") { //es de desahibilitar/habikitr
        ...
        $id_centro_costo = trim($_POST['id_centro_costo']);
        ...
        if ($modeeditstatus == "C") { ////CREACION
            $consulta = "SELECT nombre FROM u116753122_cw3completa.bodegas WHERE nombre = '$nombre'";
            $resultado = mysqli_query($conetar, $consulta);
            if (mysqli_num_rows($resultado) > 0) {
                $respuesta = 1;
                echo $respuesta;
            } else {
                $cadena = "insert into u116753122_cw3completa.bodegas(nombre,id_centro_costo,id_empleado,estado)values('" . $nombre . "','" . $id_centro_costo . "','" . $id_empleado . "',1)";
                $resultado = mysqli_query($conetar, $cadena);
            }
            ...
            if ($modeeditstatus == "E") { //acgualzucsion
                ...
                if ($modeeditstatus == "B") { //acgualzucsion
                    $cadena = "DELETE FROM u116753122_cw3completa.bodegas where id='" . $id . "'";
                    $resultado = mysqli_query($conetar, $cadena);
                }
            }
        } //De es insetar
