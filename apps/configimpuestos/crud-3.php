...
if ($conetar->connect_errno) {
    ...
    $id = trim($_POST['idiva']);
    $codiva = trim($_POST['codiva']);
    $nombreiva = trim($_POST['nombreiva']);
    $porcentajeiva = trim($_POST['porcentajeiva']);


    $cadena2="select id_iva from u116753122_cw3completa.config_iva WHERE id_iva = '$id'";
    $resultadP2=$conetar->query($cadena2);
    $numerfiles2 = mysqli_num_rows($resultadP2);
    if($numerfiles2<1){
        $cadena = "insert into u116753122_cw3completa.config_iva(codigo_iva, descripcion, porcentaje) 
        values('" . $codiva . "', '" . $nombreiva . "', '" . $porcentajeiva . "')";
        $resultado = mysqli_query($conetar, $cadena);
    }else{
        $cadena="update u116753122_cw3completa.config_iva set
                                    codigo_iva='".$codiva."',
                                    descripcion='".$nombreiva."',
