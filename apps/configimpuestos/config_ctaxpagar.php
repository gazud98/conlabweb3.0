...
if ($conetar->connect_errno) {
    ...
    $cadena = "SELECT id_ctapagar, codigo_ctapagar, descripcion, porcentaje FROM u116753122_cw3completa.config_ctaxpagar";
    //                      echo $cadena;
    $resultadP2 = $conetar->query($cadena);
    $numerfiles2 = mysqli_num_rows($resultadP2);
    if ($numerfiles2 >= 1) {
        $filaP2 = mysqli_fetch_array($resultadP2);
        $id = trim($filaP2['id_ctapagar']);
        $codigo_ctapagar = trim($filaP2['codigo_ctapagar']);
        $descripcion = trim($filaP2['descripcion']);
        $porcentaje = trim($filaP2['porcentaje']);
    }
...
?>
    ...
    <div class="content-regfical">
        <form name="form-control4" id="form-control4" action="#" class="card p-3" method="POST" enctype="multipart/form-data">
            <h5 style="text-align: center;">Configuración Cuentas Por Pagar</h5>
            <hr>
            <div class="row">
                <div class="col-md-6">
                    <input type="hidden" name="idcuenta" id="idcuenta">
                    <label><strong>Código: </strong></label>
                    <input type="input" class="formcontrol" name="codcuenta" id="codcuenta" value="" required></input>
                </div>
                <div class="col-md-6">
                    <label><strong>Nombre: </strong></label>
                    <input type="input" class="formcontrol" name="nombrecuenta" id="nombrecuenta" value="" required></input>
                </div>
            </div>
            <br>
        ...
        <section class="p-3 col-md-12">
            <div class="row pb-2 mt-3 card card-body table-config-cuenta" style="width:80%;margin:auto;overflow-x:scroll;">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('.table-config-cuenta').load('https://cw3.tierramontemariana.org/apps/configregimenfiscal/tabla-4.php');
        ...
        function buscarData3(id) {
            $.ajax({
                url: 'https://cw3.tierramontemariana.org/apps/configregimenfiscal/search-4.php?id=' + id,
                success: function(res) {
                    data = JSON.parse(res);
                    data.forEach(element => {
                        $('#idcuenta').val(element.id);
                        $('#codcuenta').val(element.cod);
                        $('#nombrecuenta').val(element.desc);
                    });
                }
            })
        }
