...
if ($conetar->connect_errno) {
    ...
    if ($modeeditstatus == "D") { //es de desahibilitar/habikitr
        ...
        $cadena = "select estado from u116753122_cw3completa.bodegaentrepanio where id='" . $id . "'";
        ...
        $cadena = "update u116753122_cw3completa.bodegaentrepanio set estado='" . $estado . "' where id='" . $id . "'";
        $resultado = mysqli_query($conetar, $cadena);
        $result = "ok";
    } else { //es crea o moifica

        $nombre = trim($_POST['nombre']);
        $stand = trim($_POST['id_stands']);
        ...
        if ($modeeditstatus == "C") { ////CREACION
            $consulta = "SELECT nombre FROM u116753122_cw3completa.bodegaentrepanio WHERE nombre = '$nombre'";
            $resultado = mysqli_query($conetar, $consulta);
            if (mysqli_num_rows($resultado) > 0) {
                $respuesta = 1;
                echo $respuesta;
            } else {
                $cadena = "insert into u116753122_cw3completa.bodegaentrepanio(nombre,idstand,estado)values('" . $nombre . "','" . $stand . "',1)";
                $resultado = mysqli_query($conetar, $cadena);
            }
            ...
            if ($modeeditstatus == "E") { //acgualzucsion
                $cadena = "update u116753122_cw3completa.bodegaentrepanio set
                                nombre='" . $nombre . "',
                                idstand='" . $stand . "'
                            where id='" . $id . "'";
                $resultado = mysqli_query($conetar, $cadena);
                ...
                if ($modeeditstatus == "B") { //acgualzucsion
                    $cadena = "DELETE FROM u116753122_cw3completa.bodegaentrepanio where id='" . $id . "'";
                    $resultado = mysqli_query($conetar, $cadena);
                }
            }
        } //De es insetar
