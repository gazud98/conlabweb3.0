...
<div class="content-filters mt-2 mb-2">
    <div class="row">
        ...
        <div class="col-md-2" style="margin-top: 30px;">
            <button type="button" class="btn btn-primary btn-sm" id="btnFilter2"><i class="fa-solid fa-filter"></i> Filtrar</button>
        </div>
    </div>
</div>

<table class="table table-striped table-head-fixed text-nowrap table-sm" style="margin-top: 2%;" id="tableVisitas">
    <thead>
        <tr>
            <th>Id</th>
            <th>Fecha y Hora</th>
...
<script>
    ...
    $(document).ready(function() {
        ...
        miDataTableVisitas = $('#tableVisitas').DataTable({
            ...
            "ajax": {
                url: 'https://cw3.tierramontemariana.org/apps/consultavisitas/mostrar-1.php?user=<?php echo $user; ?>', // Página PHP que devuelve los datos en formato JSON
            ...
            "columns": [{
                ...
                },
                {
                    ...
                    "render": function(data, type, full, meta) {
                        return '<strong>Nombre:</strong>' + full.nombre_contacto +
                            '&nbsp;&nbsp;<strong>Celular:</strong>' + full.celular_contacto +
                            '&nbsp;&nbsp;<strong>Email:</strong>' + full.email_contacto
                    }
                },
                {
                    "data": "estado",
                    "render": function(data, type, full, meta) {
                        if (full.estado == 0) {
                            ...
                            return '<span class="badge badge-dark">Cerrado</span>';
                        }
                    }
                },
                {
                    ...
                    "render": function(data, type, full, meta) {
                        ...
                        $cadena = "SELECT id_users, id_cargo FROM users WHERE id_users = '$user'";

                        $resultadP2a = $conetar->query($cadena);
                        $numerfiles2a = mysqli_num_rows($resultadP2a);
                        if ($numerfiles2a >= 1) {
                            $filaP2a = mysqli_fetch_array($resultadP2a);
                            $cargo = trim($filaP2a['id_cargo']);
                            if ($cargo == 3 && $user == $filaP2a['id_users']) {

                        ?>

                                return '<button class="btn btn-secondary btn-sm" onclick="getComments(' + full.id + ')" data-toggle="modal" data-target="#exampleNotas">Visitar <i class="fa-solid fa-paperclip"></i></button>' +
                                    '&nbsp;&nbsp;<button class="btn btn-info btn-sm" onclick="getTable(' + full.id + ')" data-toggle="modal" data-target="#modalMap">Revisar <i class="fa-solid fa-location-dot"></i></button>'
    ...
    function getComments(id) {
        ...
        $.ajax({
            type: 'POST',
            url: 'https://cw3.tierramontemariana.org/apps/consultavisitas/mostrar-2.php?aux=3&id=' + id,
