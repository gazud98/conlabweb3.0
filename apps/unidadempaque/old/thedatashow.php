...
if ($conetar->connect_errno) {
    ...
    if (!(($caso == "IP") or ($caso == "EP"))) {
        $cadena = "SELECT id_unidad_empaque, nombre,estado
            FROM  desarrollo_laboratorio.unidad_empaque
            WHERE id_unidad_empaque='" . $id . "'"; //obligfa al registo señalDO
        $resultadP2 = $conetar->query($cadena);
        $numerfiles2 = mysqli_num_rows($resultadP2);
        if ($numerfiles2 >= 1) {
            $filaP2 = mysqli_fetch_array($resultadP2);
            $id = trim($filaP2['id_unidad_empaque']);                      //RESPETAR EL $ID QES LA QUE CARGA LA LLAVE PRIMARIA
