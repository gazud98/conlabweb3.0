    ...
    <div class="wrapper">
                                        ...
                                        <select class="form-control form-control-sm" name="sede_user_default">
                                            ...
                                            <?php if (!empty($sedes)) {
                                                foreach ($sedes as $sed => $se) {  ?>
                                                    <option value="<?= $se['id_sedes'] ?>" <?= $this->auth->get_sede_default() == $se['id_sedes'] ? 'selected' : '' ?>> Sede: <?= $se['name'] ?></option>
                                            <?php } } ?>
                                        </select>
                                    </div>
                                </li>
                                        ...
                                        <div class="input-group mb-4">
                                            ...
                                            <div class="search-box w-100" style="display: block;">
                                                <form id="searchForm" action="#" role="search">
                                                    <input type="search" name="search_data" id="search_data" placeholder="Buscar Kit......" class="form-control" onkeyup="ajaxSearches();" autocomplete="off">
        ...
        <script>
            ...
            function refresh_datatables(id, name = ''){ 
                ...
                table.destroy();  get_datatables(id);
            }

            function get_datatables(sede = null){ 

                if(sede == null){ id = <?= $this->auth->get_sede_default()?>; }else{ id = sede; }
                ...
                var calendar = new FullCalendar.Calendar(calendarEl, {
                    ...
                    eventContent: function(arg, createElement) {
                        ...
                        italicEl.innerHTML = '<div class="p-1">Nº pruebas:<b> ' + pruebas + '</b><br>' +
                            ...
                            'Nº repeticiones: <b>' + verificaciones + '</b></div>';
                    ...
                    select: function(arg) {
                        kit_day(arg.startStr, id, null, date_start, name);
                    },
            ...
            function kit_live(id) {
                ...
                if (estado == 3) {
                    $('#title-prueba').text('EL Kit de '+name+' ha expirado!')
                                      ...
                                      $('#calendar').empty()
                                                    .html('<P class="lead p-3">El tiempo de vida de este kit ha llegado a su fin por favor reempacelo, recuerda cerrar este kit antes de abrir uno nuevo</p>');
                } else { start_calendar(id, date_start, name); }

            };

            function show_kit_live(id, estado, date_start, name) {
                if (estado == 3) {
                    $('#title-prueba').text('kit expirado: ¡El tiempo de vida de este kit ha llegado a su fin por favor reempacelo').removeClass('bg-primary').addClass('bg-warning');
                } else {
            ...
            function update_kit_live(kit, status) {
                ...
                if (status == 1) {
                    ...
                    alert('Este kit debe cerrarse obligatoriamente debido a ya expiró');
                }
            }
            ...
            function get_list_info_live_kit(sede = null){
                if(sede==null){ id = <?= $this->auth->get_sede_default()?>; }else{ id = sede; }
                $.post("?c=consumos&a=get_list_info_live_kit", { id: id }
                ).done(function(data) {  $('#list_kit_open').html(data)  });
            }
            ...
            function close_kit(id) {
                if (confirm('Esta seguro que desea cerrar este kit?. Recuerde que este proceso es irreversible!')) {
                    $.ajax({
            ...
            function close_day(id, kit, date_start = null) {
                if (confirm('Esta seguro que desea cerrar el dia?. Recuerde que este proceso es irreversible!')) {
                    $.ajax({
