...
const SELECTOR_CONTENT_TAB = '.tab-content'
const SELECTOR_NAV_TAB = '.nav .nav-tabs'
...
class IFrame {
  ...
  createTab(title, link, uniqueName, autoOpen) {
    ...
    $(SELECTOR_TAB_NAVBAR_NAV).append(encodeURIComponent(encodeURI(newNavItem)))

    const newTabItem = `<div class="tab-pane fade" id="${tabId}" role="tabpanel" aria-labelledby="${navId}"><iframe src="${link}"></iframe></div>`
    $(SELECTOR_TAB_CONTENT).append(encodeURIComponent(encodeURI(newTabItem)))*/

    const newNavItem2 = `<button class="nav-link" id="${tabId}" data-bs-toggle="tab" data-bs-target="#${tabId} type="button" role="tab" aria-controls="${navId}" aria-selected="true">${title}</button>`
    $(SELECTOR_NAV_TAB).append(encodeURIComponent(encodeURI(newNavItem2)))

    const newTabItem2 = `<div class="tab-pane fade" id="${tabId}" role="tabpanel" aria-labelledby="${navId}" tabindex="0"><iframe src="${link}"></iframe></div>`
    $(SELECTOR_TAB_CONTENT).append(encodeURIComponent(encodeURI(newTabItem2)))
